    ...
    <div className="min-h-screen bg-surface">
      ...
      <div className="glass-strong">
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center gap-2 text-sm">
          ...
          <span className="text-muted-light">Escorts VIP</span>
          <span className="text-border">/</span>
          <span className="text-brand font-medium">{escort.alias || escort.name}</span>
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-4 py-6">
        ...
        <div className="mb-6">
          <div className="flex items-center gap-3 flex-wrap">
            <h1 className="text-5xl md:text-7xl font-bold text-brand font-serif lowercase tracking-tight">
              {escort.alias || escort.name}
            </h1>
            {escort.verified && (
              <div className="flex items-center gap-1.5 bg-accent/10 border border-accent/30 text-accent px-3 py-1.5 rounded-full text-sm font-medium">
            ...
            {escort.featured && (
              <div className="flex items-center gap-1.5 bg-brand/5 border border-brand/20 text-brand px-3 py-1.5 rounded-full text-sm font-medium">
                <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
          ...
          {escort.price && (
            <div className="mt-2 flex items-center gap-2">
              <span className="text-2xl font-bold text-accent font-serif">{formatPrice(escort.price)}</span>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-8">
          {escort.atHome && (
            <div className="flex items-center gap-2.5 glass-card bg-surface-alt/50 px-4 py-3">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-1">
            <div
              className="relative aspect-[3/4] bg-surface-container rounded-none overflow-hidden cursor-pointer group"
            ...
            <div className="mt-6 grid grid-cols-3 gap-2">
              ...
              {escort.measurements && (
                <div className="glass-card px-3 py-2 text-center col-span-2">
              ...
              <div className="mt-6">
                ...
                <div className="space-y-1.5">
                  {Object.entries(availability).map(([day, hours]) => (
                    <div key={day} className="flex justify-between items-center glass-card px-3 py-2">
          ...
          <div className="lg:col-span-2 space-y-8">
            {escort.description && (
              <div className="glass-card bg-surface-alt/50 border border-border rounded-none p-6">
                <h2 className="text-xl font-bold text-brand mb-4 font-serif">Sobre Mí</h2>
            ...
            {/* Contact */}
            <div className="glass-card border border-border rounded-none p-6">
              <h2 className="text-xl font-bold text-brand mb-4 font-serif">Contacto</h2>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {escort.whatsapp && (
                  <a
                    ...
                    className="flex items-center justify-center gap-2 bg-rose hover:bg-rose-dark text-brand font-bold py-3.5 px-4 rounded-sm transition-all hover:scale-[1.02]"
                  >
                ...
                {escort.phone && (
                  <a
                    ...
                    className="flex items-center justify-center gap-2 bg-rose/60 hover:bg-rose-dark text-brand font-bold py-3.5 px-4 rounded-sm transition-all hover:scale-[1.02]"
                  >
            ...
            {escort.reviews.length > 0 && (
              <div className="glass-card bg-surface-alt/50 border border-border rounded-none p-6">
                <div className="flex items-center justify-between mb-4">
                  <h2 className="text-xl font-bold text-brand font-serif flex items-center gap-2">
                ...
                <div className="space-y-4">
                  {escort.reviews.slice(0, 3).map((review) => (
                    <div key={review.id} className="glass-overlay border border-border rounded-none p-4">
            ...
            {services.length > 0 && (
              <div className="glass-card bg-surface-alt/50 border border-border rounded-none p-6">
                <h2 className="text-xl font-bold text-brand mb-4 font-serif">Mis Servicios</h2>
                <div className="flex flex-wrap gap-2">
                  {services.map((service: string, i: number) => (
                    <span
                      key={i}
                      className="bg-surface-container text-muted px-4 py-2 rounded-none text-sm font-medium transition-colors cursor-default border border-border"
                    >
                      {service}
                    </span>
                  ))}
            ...
            {/* Attributes */}
            <div className="glass-card border border-border rounded-none p-6">
              <h2 className="text-xl font-bold text-brand mb-4 font-serif">Mis Atributos</h2>
              <div className="flex flex-wrap gap-2">
                {[
                  escort.nationality,
                  escort.bodyType,
                  ...
                  <span
                    key={i}
                    className="bg-surface text-muted px-3 py-1.5 rounded-none text-sm border border-border"
                  >
                    {attr}
                  </span>
                ))}
        ...
        {allMedia.length > 0 && (
          <div className="mt-10">
            <div className="glass-card border border-border rounded-none p-6">
              <h2 className="text-xl font-bold text-brand mb-4 font-serif">Mis Fotos & Videos</h2>
              {escort.verified && (
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {allMedia.slice(1).map((media, idx) => (
                  <div
                    key={idx}
                    className="relative aspect-[3/4] bg-surface-container rounded-none overflow-hidden border border-border cursor-pointer group"
      ...
      {lightboxOpen && (
        ...
        >
          ...
          {allMedia.length > 1 && (
            <>
              <button
                className="absolute left-4 top-1/2 -translate-y-1/2 glass-dark text-white/80 hover:text-white p-2 rounded-full z-10"
              ...
              <button
                className="absolute right-4 top-1/2 -translate-y-1/2 glass-dark text-white/80 hover:text-white p-2 rounded-full z-10"
          ...
          <div
            className="relative w-full max-w-4xl h-[80vh] mx-4 flex items-center justify-center"
          ...
          <div className="absolute bottom-6 left-1/2 -translate-x-1/2 text-white/60 text-sm flex items-center gap-2">
            {allMedia[lightboxIndex]?.type === 'video' && (
